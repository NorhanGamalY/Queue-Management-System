...
function QueueManagementTab({ businessId }) {
  ...
  const handleCallNext = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/v1/tickets/${nextTicket._id}/call`, {
        method: 'PUT',
        credentials: 'include',
      });
  ...
  const handleServeTicket = async (ticketId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/v1/tickets/${ticketId}/serve`, {
        method: 'PUT',
        credentials: 'include',
      });
  ...
  const handleCancelTicket = async (ticketId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/v1/tickets/${ticketId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
