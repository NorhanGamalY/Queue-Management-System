...
export default function PatientDashboard() {
    ...
    const handleSearch = async () => {
        ...
        setIsSearching(true);
        try {
            const response = await fetch(`http://localhost:5000/api/v1/search?q=${encodeURIComponent(searchQuery)}`);
    ...
    useEffect(() => {
        const fetchMyTickets = async () => {
            ...
            try {
                const res = await fetch('http://localhost:5000/api/v1/users/me/tickets', {
                    credentials: 'include',
                });
                if (res.ok) {
                    const data = await res.json();
    ...
    useEffect(() => {
        const fetchMyReviews = async () => {
            ...
            try {
                const res = await fetch('http://localhost:5000/api/v1/users/me/reviews', {
                    credentials: 'include',
                });
                if (res.ok) {
                    const data = await res.json();
    ...
    useEffect(() => {
        const fetchMyPayments = async () => {
            ...
            try {
                const res = await fetch('http://localhost:5000/api/v1/users/me/payments', {
                    credentials: 'include',
                });
                if (res.ok) {
                    const data = await res.json();
                        ...
                        <button 
                          onClick={async () => {
                            if (confirm('Are you sure you want to cancel this ticket?')) {
                              try {
                                const res = await fetch(`http://localhost:5000/api/v1/tickets/${ticket._id}/cancel`, {
                                  method: 'PUT',
                                  credentials: 'include',
