...
export default function PatientDashboard() {
  ...
  useEffect(() => {
    if (activeTab === 'findBusiness') {
      const fetchBusinesses = async () => {
        setIsSearching(true);
        try {
          const response = await fetch('http://localhost:5000/api/v1/search/businesses?limit=20');
          const data = await response.json();
          setSearchResults(data.data?.businesses || []);
        } catch (error) {
  ...
  useEffect(() => {
    if (activeTab === 'myTickets' || activeTab === 'appointments') {
      ...
      // Auto-refresh tickets every 3 seconds when on tickets tab
      const refreshInterval = setInterval(fetchMyTickets, 3000);
  ...
  useEffect(() => {
    if (activeTab === 'reviews') {
      const fetchMyReviews = async () => {
        ...
        try {
          const res = await fetch('http://localhost:5000/api/v1/reviews/users/me/reviews', {
            credentials: 'include',
          });
          if (res.ok) {
  ...
  useEffect(() => {
    if (activeTab === 'payments') {
      const fetchMyPayments = async () => {
        ...
        try {
          const res = await fetch('http://localhost:5000/api/v1/payments/users/me/payments', {
            credentials: 'include',
          });
          if (res.ok) {
            const result = await res.json();
  ...
  useEffect(() => {
    ...
    socket.on('yourTurn', (ticket) => {
      toast.success(`It's your turn! Ticket #${ticket.ticketNumber || ticket.position}`, {
        duration: 5000,
      });
      if (Notification.permission === 'granted') {
        new Notification("It's Your Turn!", {
          body: `Please proceed to ${ticket.businessId?.name}`,
          icon: '/logo.png',
        });
      }
    });
