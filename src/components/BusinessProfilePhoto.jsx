...
export default function BusinessProfilePhoto({ businessData, isDisabled = true, onPhotoUpdated }) {
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/v1/businesses/upload-profile-photo', {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
  ...
  return (
    <div className="relative w-32 h-32 rounded-full overflow-hidden border-4 border-[#359487] dark:border-[#C6FE02] bg-gray-100 dark:bg-gray-800">
      {profileImageUrl ? (
        <img
          src={profileImageUrl}
