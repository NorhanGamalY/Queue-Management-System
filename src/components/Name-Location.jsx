...
const NameLocation = async ({index}) => {
    ...
    return (
        <div className=''>
            <h3 className='text-[20px] dark:text-white'>{data[index].businessName}</h3>
            <p className='flex items-center'><span className='me-1'>{<CiLocationOn />}</span>{data[index].location}</p>
        </div>
    );
}
