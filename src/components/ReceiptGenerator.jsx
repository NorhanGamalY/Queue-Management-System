...
export default function ReceiptGenerator({ receiptData }) {
  ...
  const handleDownload = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/v1/payment/download-receipt', {
        method: 'POST',
        headers: {
  ...
  const handleEmailReceipt = async () => {
    setSending(true);
    try {
      const response = await fetch('http://localhost:5000/api/v1/payment/email-receipt', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div className="bg-white dark:bg-[#2b2825] rounded-2xl shadow-lg overflow-hidden">
      {/* Action Buttons */}
      <div className="bg-gray-50 dark:bg-[#221F1B] px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={sending}
            className="px-4 py-2 rounded-lg bg-[#359487] dark:bg-[#C6FE02] text-white dark:text-black hover:bg-[#2a8074] dark:hover:bg-[#a7d404] transition-colors flex items-center gap-2 disabled:opacity-50"
          >
            <Mail size={16} />
      ...
      <div ref={receiptRef} className="p-8 max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8 pb-6 border-b-2 border-gray-200 dark:border-gray-700">
          <h1 className="text-3xl font-bold text-[#359487] dark:text-[#C6FE02] mb-2">
            QuickQueue
          </h1>
        ...
        <div className="flex justify-end mb-8">
          <div className="w-64 space-y-3">
            ...
            <div className="flex justify-between text-xl font-bold text-gray-800 dark:text-white pt-3 border-t-2 border-gray-300 dark:border-gray-600">
              <span>Total:</span>
              <span className="text-[#359487] dark:text-[#C6FE02]">${total?.toFixed(2)}</span>
            </div>
          </div>
        </div>

        {/* Payment Method */}
        <div className="bg-gray-50 dark:bg-[#221F1B] rounded-lg p-4 mb-6">
